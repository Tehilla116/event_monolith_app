...
export const eventRoutes = new Elysia({ prefix: "/events" })
  ...
  .put(
    ...
    async ({ headers, set, params, body, jwt }: any) => {
      console.log("📝 Received event update request for:", params.id);
      ...
      try {
        ...
        console.log("User updating event:", user.email);
        ...
        console.log("✅ Event updated successfully:", params.id);
        return {
          message: result.message,
          event: result.event,
        };
  ...
  .delete(
    "/:id",
    async ({ headers, set, params, jwt }: any) => {
      console.log("📝 Received event delete request for:", params.id);
      ...
      try {
        ...
        console.log("User deleting event:", user.email);
        ...
        // Delete event
        const result = await deleteEvent(params.id, user.id, user.role);
        set.status = result.status;

        if (!result.success) {
          console.error("❌ Event delete failed:", result.error);
          return {
            error: result.error,
          };
        }
